...
INSERT INTO CATEGORY (category_name, description, icon_name, hex_color)
SELECT v.n, v.d, v.i, v.h FROM (VALUES
                                    (N'Điện thoại',           N'Điện thoại thông minh các hãng', 'Smartphone', '#3b82f6'),
                                    (N'Laptop',               N'Máy tính xách tay',              'Laptop',     '#10b981'),
                                    (N'Máy tính bảng',        N'Tablet đa dạng kích thước',       'Tablet',     '#f59e0b'),
                                    (N'Tai nghe',             N'Tai nghe có dây và không dây',    'Headphones', '#8b5cf6'),
                                    (N'Đồng hồ thông minh',  N'Smartwatch theo dõi sức khỏe',   'Watch',      '#ef4444')
                               ) AS v(n, d, i, h)
WHERE NOT EXISTS (SELECT 1 FROM CATEGORY c WHERE c.category_name = v.n);

-- 3. BRAND (unique: brand_name)
INSERT INTO BRAND (brand_name, country, description, logo_url, is_partner)
SELECT v.n, v.c, v.d, v.l, v.p FROM (VALUES
                                         ('Apple',   'USA',   N'Thương hiệu công nghệ số 1 thế giới', 'https://upload.wikimedia.org/wikipedia/commons/f/fa/Apple_logo_black.svg', 1),
                                         ('Samsung', 'Korea', N'Tập đoàn điện tử Hàn Quốc',           'https://upload.wikimedia.org/wikipedia/commons/2/24/Samsung_Logo.svg',     1),
                                         ('Xiaomi',  'China', N'Thương hiệu công nghệ giá rẻ',        'https://upload.wikimedia.org/wikipedia/commons/a/ae/Xiaomi_logo_%282021-%29.svg', 1),
                                         ('Sony',    'Japan', N'Tập đoàn điện tử Nhật Bản',           'https://upload.wikimedia.org/wikipedia/commons/c/ca/Sony_logo.svg',       0),
                                         ('Dell',    'USA',   N'Nhà sản xuất máy tính Mỹ',            'https://upload.wikimedia.org/wikipedia/commons/1/18/Dell_logo_2016.svg',  0)
                                    ) AS v(n, c, d, l, p)
...
-- 5. STORE_BRANCH
INSERT INTO STORE_BRANCH (branch_name, location, manager_name, contact_number, address, working_hours, maps_url)
SELECT v.n, v.l, v.m, v.c, v.a, v.w, v.mu FROM (VALUES
                                                    (N'Chi nhánh Hà Nội',  N'Hoàn Kiếm, Hà Nội',  N'Phạm Minh Đức',  '0243000001', N'25 Tràng Tiền, Hoàn Kiếm, Hà Nội',  '08:00 - 22:00', 'https://maps.app.goo.gl/hanoi1'),
                                                    (N'Chi nhánh TP.HCM',  N'Quận 1, TP.HCM',     N'Nguyễn Thị Mai', '0283000002', N'100 Lê Lợi, Q.1, TP.HCM',           '08:00 - 22:00', 'https://maps.app.goo.gl/hcm1'),
                                                    (N'Chi nhánh Đà Nẵng', N'Hải Châu, Đà Nẵng',  N'Trần Thanh Hải', '0236300003', N'321 Nguyễn Văn Linh, Đà Nẵng',      '08:00 - 21:00', 'https://maps.app.goo.gl/danang1'),
                                                    (N'Chi nhánh Cần Thơ', N'Ninh Kiều, Cần Thơ',  N'Lê Hoàng Anh',   '0292000004', N'56 Hoà Bình, Ninh Kiều, Cần Thơ',   '08:00 - 21:00', 'https://maps.app.goo.gl/cantho1')
                                               ) AS v(n, l, m, c, a, w, mu)
...
-- 9. PRODUCT
INSERT INTO PRODUCT (product_name, description, price, original_price, discount_percent, rating, sold_count, category_id, brand_id, status, created_date, supplier_id)
SELECT v.pn, v.descr, v.price, v.op, v.dp, v.rt, v.sc, v.cat, v.brd, v.st, v.cdate, v.sup FROM (VALUES
                                                                                                           (N'iPhone 15 Pro 256GB', N'iPhone thế hệ 15 Pro, chip A17, camera 48MP', 29990000, 32990000, 10, 4.8, 150, (SELECT category_id FROM CATEGORY WHERE category_name = N'Điện thoại'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Apple'),  'AVAILABLE', '2025-01-10 08:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'supply@techviet.vn')),
                                                                                                           (N'Samsung Galaxy S24 Ultra 512GB', N'Samsung flagship 2024, camera 200MP', 27990000, 29990000, 7, 4.7, 120, (SELECT category_id FROM CATEGORY WHERE category_name = N'Điện thoại'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Samsung'), 'AVAILABLE', '2025-01-15 09:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'contact@digisource.vn')),
                                                                                                           (N'Xiaomi 14 256GB', N'Leica camera, Snapdragon 8 Gen 3', 16990000, 18990000, 11, 4.5, 200, (SELECT category_id FROM CATEGORY WHERE category_name = N'Điện thoại'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Xiaomi'),  'AVAILABLE', '2025-02-01 10:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'info@smartimport.vn')),
                                                                                                           (N'MacBook Pro 14 inch M3 Pro', N'Chip M3 Pro, RAM 18GB, SSD 512GB', 52990000, 56990000, 7, 4.9, 45, (SELECT category_id FROM CATEGORY WHERE category_name = N'Laptop'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Apple'),  'AVAILABLE', '2025-01-20 08:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'supply@techviet.vn')),
                                                                                                           (N'Dell XPS 15 9530', N'Intel Core i9 13th, RAM 32GB, SSD 1TB', 43990000, 48990000, 10, 4.6, 30, (SELECT category_id FROM CATEGORY WHERE category_name = N'Laptop'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Dell'), 'AVAILABLE', '2025-02-10 09:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'contact@digisource.vn')),
                                                                                                           (N'Samsung Galaxy Book4 Pro 360', N'Laptop 2-in-1 AMOLED, Core Ultra 7', 32990000, 35990000, 8, 4.7, 25, (SELECT category_id FROM CATEGORY WHERE category_name = N'Laptop'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Samsung'),  'AVAILABLE', '2025-03-01 10:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'info@smartimport.vn')),
                                                                                                           (N'Sony WH-1000XM5', N'Tai nghe chống ồn hàng đầu, Bluetooth 5.2', 8990000, 9990000, 10, 4.9, 300, (SELECT category_id FROM CATEGORY WHERE category_name = N'Tai nghe'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Sony'),  'AVAILABLE', '2025-01-05 08:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'supply@techviet.vn')),
                                                                                                           (N'Apple AirPods Pro 2', N'Chip H2, chống ồn cải tiến, sạc USB-C', 6290000, 6990000, 10, 4.8, 500, (SELECT category_id FROM CATEGORY WHERE category_name = N'Tai nghe'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Apple'),  'AVAILABLE', '2025-02-20 09:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'contact@digisource.vn')),
                                                                                                           (N'Apple Watch Series 9 45mm', N'Chip S9, màn hình always-on', 12990000, 13990000, 7, 4.7, 80, (SELECT category_id FROM CATEGORY WHERE category_name = N'Đồng hồ thông minh'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Apple'),  'AVAILABLE', '2025-01-25 10:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'info@smartimport.vn')),
                                                                                                           (N'Samsung Galaxy Watch 7 44mm', N'Exynos W1000, đo ECG, pin 40h', 8490000, 9490000, 11, 4.6, 60, (SELECT category_id FROM CATEGORY WHERE category_name = N'Đồng hồ thông minh'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Samsung'),  'AVAILABLE', '2025-03-15 11:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'supply@techviet.vn')),
                                                                                                           (N'iPhone 13 128GB', N'iPhone 13 cũ, chip A15 Bionic', 14990000, 16990000, 12, 4.4, 1000, (SELECT category_id FROM CATEGORY WHERE category_name = N'Điện thoại'), (SELECT brand_id FROM BRAND WHERE brand_name = 'Apple'), 'DISCONTINUED', '2024-06-01 08:00:00', (SELECT supplier_id FROM SUPPLIER WHERE email = 'supply@techviet.vn'))
                                                                                                      ) AS v(pn, descr, price, op, dp, rt, sc, cat, brd, st, cdate, sup)
